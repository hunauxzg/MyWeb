...
var prompt = (function() {
	...
	return (function Prompt(type, describe, time) {
		...
		$('.prompt').fadeIn(time, function() {
			$('.prompt').fadeOut(time, function() {
				$('.prompt').remove();
			})
		})
	})
...
function fn_commonOpenConvInfoW(mapObj, pointXY, overLayArr) {
	...
	var dw=250,dh=204,uw=250,uh=204,ww=280,wh=210; //div宽高，ul宽高，信息窗宽高。
	...
	var htmlStr = 	'<div class="convClkShowDiv" style="display:none;width:' + dw + 'px;height:' + dh + 'px;">' +
					'<ul class="clearfix" style="width:' + uw + 'px;height:' + uh + 'px;">';
	...
	var infoWindow = new BMap.InfoWindow(infoWinDom[0],{width:ww,height:wh});
...
function fn_commonUserAndGroupChoose(options) {
	...
	// 创建人员选择对话框面板。
	var htmlStr = 	'<div class="commonMemberChoose">' +
					'    <div class="float-left">' +
					'	    <div class="dicTitle">'+lang.common.allUsers+'</div>' +
					'	    <div class="search">' +
					'	        <input type="text" placeholder="'+lang.dispatch.memberOrName+'" id="searchAllMember"/><div class="icon">&#xe60d;</div>' +
					'	    </div>' +
					'	    <div>' +
					'	        <div class="treeWrap"><ul id="treeAllMember"></ul></div>' +
					'	        <div class="btn icon" id="addSelMember">&#xe6a9; '+lang.dispatch.addObject+'</div>' +
					'	    </div>' +
					'	</div>' +
					'	<div class="float-right">' +
					'	    <div class="dicTitle">'+lang.dispatch.beSelectd+' <span style="color:#f5f5f5">' + _this.config.gname + '</span></div>' +
					'	    <div class="search">' +
					'	        <input type="text" placeholder="'+lang.system.PleaseNameOrNumber+'" id="searchSelMember"/><div class="icon">&#xe60d;</div>' +
					'	    </div>' +
					'	    <div class="rightTable"><table id="selMemberTable"></table></div>' +
					'	    <div class="btn icon" id="delSelMember">&#xe606; '+lang.dispatch.cleanObject+'</div>' +
					'	</div>' +
					'	<div class="footer">' +
					'	    <img src="static/img/true.png"  id="MemberChooseOK"  alt=""/>' +
					'	    <img src="static/img/false.png" id="MemberChooseQX"  alt=""/>' +
					'	</div>' +
					'</div>';
	...
	this.ctrlClick = function() {
	    $(document).keydown(function (e) {
	        if (e.keyCode === 17) {
	            $('.commonMemberChoose .treeWrap .tree-node').off('click').on('click', function (e) {
	...
	this.treeInit = function() {
		$('.commonMemberChoose #treeAllMember').tree({
			...
			onLoadSuccess : function(node, data) {
	            ...
	            $.each( $('.commonMemberChoose .treeWrap .tree-node'), function () {
	...
	// 树形菜单小组查询 回车键盘事件
	$(".commonMemberChoose #searchAllMember").off('keydown').on('keydown',function(evt) {
	...
	// 已选成员查询， 回车键盘事件
	$(".commonMemberChoose #searchSelMember").off('keydown').on('keydown',function(evt) {
		evt = (evt) ? evt : ((window.event) ? window.event : "");
	...
	//添加成员按钮点击事件。
	$('.commonMemberChoose #addSelMember').off('click').on('click',function(event) {
		var gnum = _this.config.gnum;
	...
	//表格已选成员清空。
	$('.commonMemberChoose #delSelMember').off('click').on('click',function(event) {
		var curRows = $('.commonMemberChoose #selMemberTable').datagrid('getRows');
	...
	//表格移出按钮点击事件。
	$('.commonMemberChoose .rightTable').off('click').on('click',function(e) {
	...
	//关闭按钮点击事件。
	$('.commonMemberChoose #MemberChooseQX').off('click').on('click',function(event) {
		$('.commonMemberChoose').dialog('close');
	});
	//确定按钮点击事件。
	$('.commonMemberChoose #MemberChooseOK').off('click').on('click',function(event) {
		$('.commonMemberChoose').dialog('close');
...
function fn_commonSetPosition(options) {
	...
	$.extend(_this.config, options);
	// 创建地图们置选择对话框面板。
	var htmlStr = 	'<div class="commonMapSite" style="display:none">' +
					'	<div class="mapSearchDiv">' +
					'		<input type="text" id="mapSearch" placeholder="'+lang.prompt.inputLocationSearch+'"/>' +
					'		<span class="icon">&#xe64a;</span>' +
					'	</div>' +
					'	<div class="map" id="commonChooseMap"></div>' +
					'	<div class="coordinate">' +
					'		<div class="float-left">' +
					'			<div class="float-left">'+lang.prompt.latAndLng+' :</div>' +
					'			<div class="float-right"><input type="text" style="width:220px;" id="mapXY"/></div>' +
					'		</div>' +
					'		<div class="float-right">' +
					'			<div class="float-left">'+lang.prompt.locationName+' :</div>' +
					'			<div class="float-right"><input type="text" style="width:310px;" id="mapName"/></div>' +
					'		</div>' +
					'	</div>' +
					'	<div class="describe">' +
					'		<div class="float-left">'+lang.prompt.descriptionInfo+' :</div>' +
					'		<div class="float-right"><input type="text" style="width:529px;" id="mapDesc"/></div>' +
					'	</div>' +
					'	<div class="footer">' +
					'		<img src="static/img/true.png"  alt="" id="setMapInfo"/>' +
					'		<img src="static/img/false.png" alt="" id="offMapInfo"/>' +
					'	</div>'+
					'</div>';
	var isInit = false;
...
//		map.setCurrentCity("杭州");       //设置地图显示的城市，三维地图此项必须设置。
		...
		//关闭按钮点击事件。
		$('.commonMapSite #offMapInfo').off('click').on('click',function(event) {
		...
		//确定按钮点击事件。
		$('.commonMapSite #setMapInfo').off('click').on('click',function(event) {
			$('.commonMapSite').dialog('close');
...
//		mapEnv = 'offline';
		...
		var map = null;
		if (mapEnv == 'online') {
			map = new google.maps.Map($('.commonMapSite #commonChooseMap')[0], {
				zoom: Number(mapInitLevel),
				center: {lng: Number(mapInitPointArr[0]), lat: Number(mapInitPointArr[1])},
				streetViewControl: false, //是否显示街景
		...
		} else if (mapEnv == 'offline') {
			function LocalMapType() {}
			LocalMapType.prototype.tileSize = new google.maps.Size(256, 256);
			LocalMapType.prototype.maxZoom = 19; // 地图的最大显示级别
			LocalMapType.prototype.minZoom = 0; // 地图的最小显示级别
			...
			LocalMapType.prototype.getTile = function(coord, zoom, ownerDocument) {
				...
				var strURL = '/googlemap/tiles/'; // 本地地图切片存放路径
				strURL += zoom + "/" + coord.x + "/" + coord.y + imgExt;
				img.src = strURL;
				return img;
			};
		...
		function showInfo(e) {
			var lng = e.latLng.lng().toFixed(6);  //经
			var lat = e.latLng.lat().toFixed(6);  //纬
			var wgsPointArr = gcjtowgs(Number(lng), Number(lat));
			var mapXYVal = Number(wgsPointArr[0]).toFixed(6) + ',' + Number(wgsPointArr[1]).toFixed(6);
			$(".commonMapSite #mapXY").val(mapXYVal);
			setTimeout(function() {
				// 首先清除地图现有标记。
				if (window.commSetMap._marker) {
					window.commSetMap._marker.setMap(null);
				}
				var curPointXY = new google.maps.LatLng(Number(lat), Number(lng));
				var marker = new google.maps.Marker({
				    position: curPointXY,
				    map: window.commSetMap
				});
				window.commSetMap._marker = marker;
				window.commSetMap.panTo(curPointXY);
			}, 500);

			if (mapEnv == 'online') { //如果是在线地图，才能得出当前位置的具体地址名称。
	...
	setTimeout(function() {
		...
		if (point.length == 2) {
			var glatlng = wgs2gcj(Number(point[1]), Number(point[0]));
			var curPointXY = new google.maps.LatLng(Number(glatlng[0]), Number(glatlng[1]));
			var marker = new google.maps.Marker({
			    position: curPointXY,
			    map: window.commSetMap
			});
...
function fn_offLineAutoComplete(jqObj, callbackFn) {
    ...
    jqObj.off('input').on('input', function(event) {
        ...
        fn_querySuggestAddr(searchMessage, nowTimeStamp, function(jsonArr) {
	    ...
	    //hover效果
	    $ulTags.off('mouseenter').on('mouseenter','li',function(event){
...
//	        getMessageValueit(liIndex);
	    ...
	    $ulTags.off('mouseleave').on('mouseleave','li',function(event){
	    ...
	    //点击li阻止默认事件
	    $ulTags.off('click').on('click','li',function (event) {
	    	//继续添加操作
	    ...
	    function searchUpandDown(nowCount) {
	        ...
	        jqObj.parent().off('keydown').on('keydown',function(event) {
...
function dgColCvt(dictCode, bgBar) {
	...
	return function(value,row,index) {
		...
		for (var i = 0; i < dictData.length; i++) {
			if (dictData[i].ITEM_CODE && dictData[i].ITEM_CODE == value){
				if (dictData[i].BGCOLOR && dictData[i].BGCOLOR!='' && bgBar && bgBar == true) {
					return boult(dictData[i].ITEM_NAME,dictData[i].BGCOLOR);
				} else {
...
$.extend($.fn.validatebox.defaults.rules, {
	...
	checkOrgName : {
		validator : function(value) {
			...
			$.ajax({
				type : "POST",
				async : false,
				url : 'webservice/api/v1/comm/orginfo/checkName',
				dataType : "json",
				data : {
					"name" : value,
					"num" : num
		...
		message : lang.prompt.unitNameExists
	},
	//只能输入数字
	checkNumber : {
		validator : function(value) {
		...
		message : lang.prompt.inputPureNumber
	},
	//只能输入数字与*号
	checkTermNum : {
		validator : function(value) {
		...
		message : lang.prompt.inputPureStarNumber
	},
	// 检查结束号码大于等于开始号码加上数量。
	checkGEQSN : {
		validator : function(value, param) {
			var start = $(param[0]).numberbox('getValue');//开始号码段
			var num = $(param[1]).numberbox('getValue');//允许数量。
			var count = parseInt(start) + parseInt(num) -1;
			var valNum = parseInt(value);
			if (start < valNum && count <= valNum) {
				return true;
			} else {
				return false;
			}
		},
		message : lang.prompt.endGreaterStart
	},
	//检查值是否为其它几个字段值的总和。
	checkSum : {
		validator : function(value,param) {
			...
			for (var i = 0; i < param.length; i++) {
				var num = $(param[i]).numberbox('getValue');
				sum += parseInt(num);
			}
			if (sum == value) {
				return true;
		...
		message : lang.prompt.totalUserNumber
	},
	checkBetween : {
		validator : function(value,param) {
			var num = parseInt(value);
			var startNum =  $(param[0]).numberbox('getValue');
			var endNum =  $(param[1]).numberbox('getValue');
		...
		message : lang.prompt.administratorNotInConsole
	},
	checkName: {
		validator: function (value) {
			return /^[\u0391-\uFFE5\w\.\,\s ]+$/.test(value);
		},
		message: lang.prompt.onlyCnEnAllowed
	},
	flEmpty: {
        validator: function (value, param) {
            var reg=/^[\s　]|[ ]$/gi;
        ...
        message: lang.prompt.noSpaces
    },
	checkPwd: {
		validator: function (value) {
			return /^[^\u4e00-\u9fa5]{0,}$/.test(value);
		},
		message: lang.prompt.passwordNotChinese
	},
	range: {
		validator:function(value,param) {
			if (/^[0-9]\d*$/.test(value)) {
		...
		message:lang.prompt.numberBetweenAnd
	},
	checkLen: {
		validator: function (value, param) {
			var cArr = value.match(/[^\x00-\xff]/ig);
			var strLen = value.length + (cArr == null ? 0 : cArr.length);
			if (strLen >= param[0] && strLen <= param[1]) return true;
			return false;
		},
		message: lang.prompt.stringBetweenAnd
	},
	equaldDate: {
		validator: function (value, param) {
		...
		message: lang.prompt.endGreaterStartDate //匹配失败消息
	},
	checkDate: {
		validator: function (value) {
		...
		message: lang.prompt.dateNotLegal //匹配失败消息
	},
	checkDateTime: {
		validator: function (value) {
			if (!value || value == '') return true;
		...
		message: lang.prompt.dateTimeNotLegal //匹配失败消息
	},
	ip : {// 验证IP地址
		validator : function(value) {
			if (!value || value == '') return true;
		...
		message : lang.prompt.IPaddressIncorrect
	},
	idcard : {// 验证身份证
		validator : function(value) {
			if (!value || value == '') return true;
		...
		message : lang.prompt.IDcardIncorrect
	},
	carNo : {
		validator : function(value) {
			if (!value || value == '') return true;
			return /^[\u4E00-\u9FA5][\da-zA-Z]{1}.[\da-zA-Z]{5}$/.test(value);
		},
		message : lang.prompt.invalidLicensePlate
	},
	phoneAndMobile : {// 电话号码或手机号码
		...
		message : lang.prompt.phoneIncorrect
	},
	checkLtGt : {// 验证特殊符号<>
		validator: function (value) {
			if (!value || value == '') return true;
		...
		message: lang.prompt.noSpecialSymbols
	}
});

// 校验备注字段内容是否符合要求，
function checkMemoVal(value, maxLen) {
	if (/[<>]/i.test(value)) {
		prompt('error', '备注内容中不允许含有<>等特殊符号。', 1000);
		return false;
	}
	var cArr = value.match(/[^\x00-\xff]/ig);
	var strLen = value.length + (cArr == null ? 0 : cArr.length);
	if (strLen > maxLen) {
		prompt('error', '备注内容超出所允许的最大长度。', 1000);
		return false;
	}
	return true;
}
...
function fn_getMinCanUseGNum() {
	...
	$.ajax({
		type : "POST",
		url : 'webservice/api/v1/comm/orginfo/getOAStat',
		async : false,
		data : {},
		dataType : "json",
		success : function(data) {
