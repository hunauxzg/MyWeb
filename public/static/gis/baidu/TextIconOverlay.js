...
(function() {
	...
	(function() {
		...
		baidu.dom.getPosition = function(element) {
			...
			// 对于这些有bug的情况
			// 使用递归查找的方式
			BUGGY_GECKO_BOX_OBJECT = browser.isGecko > 0 && doc.getBoxObjectFor && getStyle(element, 'position') == 'absolute'
	 ...
	 *            text 表示该覆盖物显示的文字信息。
	 * @param {Json
	 *            Object} options 可选参数，可选项包括：<br /> "<b>styles</b>":{Array<IconStyle>}
	 *            一组图标风格。单个图表风格包括以下几个属性：<br />
	 *            url {String} 图片的url地址。(必选)<br />
	 *            size {Size} 图片的大小。（必选）<br />
	 *            anchor {Size} 图标定位在地图上的位置相对于图标左上角的偏移值，默认偏移值为图标的中心位置。（可选）<br />
	 *            offset {Size}
	 *            图片相对于可视区域的偏移值，此功能的作用等同于CSS中的background-position属性。（可选）<br />
	...
	TextIconOverlay.prototype._buildCssText = function(style) {
		...
		csstext
				.push('cursor:pointer; color:' + textColor + '; position:absolute; font-size:' + textSize
	 ...
	 * @example <b>参考示例：</b><br />
	 * 
	 * myTextIconOverlay.addEventListener("mouseover", function(e) {
	 * alert(e.point); });
	 * 
	 */
