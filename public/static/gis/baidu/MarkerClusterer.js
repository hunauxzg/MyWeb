...
(function() {
	 ...
	 *            聚合点的落脚位置是否是所有聚合在内点的平均值，默认为否，落脚在聚合内的第一个点<br />
	 ...
	 * 
	 * @param {Array
	 *            <BMap.Marker>} markers 需要被删除的marker数组
	 * 
	 * @return {Boolean} 删除成功返回true，否则返回false
	 */
	...
	Cluster.prototype.updateClusterMarker = function() {
		...
		var thatPoint = this._center;
		var thatBounds = this.getBounds();
		var thatMarkers = this._markers;
		if (this._markers.length == this._minClusterSize) {
			this._clusterMarker.addEventListener("click", function(event) {
				console.log(thatMarkers);
//				thatMap.setViewport(thatBounds);
				fn_commonOpenConvInfoW(thatMap, thatPoint, thatMarkers);
			});
		}
	};
